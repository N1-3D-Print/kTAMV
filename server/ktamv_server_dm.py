...
# ----------------- TAMV Nozzle Detection as tested in ktamv_cv -----------------
    ...
    def nozzleDetection(self, image):
        ...
        if(keypoints is not None and len(keypoints) >= 1):
            ...
            if len(keypoints) > 1:
                ...
                cx, cy, _, _ = self.get_frame_center(nozzleDetectFrame)
                closest_index = self.find_closest_keypoint(keypoints, np.array([cx, cy]))
            ...
            keypointRadius = 17
            cx, cy, _, _ = self.get_frame_center(nozzleDetectFrame)
            nozzleDetectFrame = cv2.circle(img=nozzleDetectFrame, center=(cx,cy), radius=keypointRadius, color=(0,0,0), thickness=3,lineType=cv2.LINE_AA)
            nozzleDetectFrame = cv2.circle(img=nozzleDetectFrame, center=(cx,cy), radius=keypointRadius+1, color=(0,0,255), thickness=1,lineType=cv2.LINE_AA)
            center = None
        # draw crosshair
        cx, cy, width, height = self.get_frame_center(nozzleDetectFrame)
        nozzleDetectFrame = cv2.line(nozzleDetectFrame, (cx,0), (cx,height), (0,0,0), 2)
        nozzleDetectFrame = cv2.line(nozzleDetectFrame, (0,cy), (width,cy), (0,0,0), 2)
        nozzleDetectFrame = cv2.line(nozzleDetectFrame, (cx,0), (cx,height), (255,255,255), 1)
        nozzleDetectFrame = cv2.line(nozzleDetectFrame, (0,cy), (width,cy), (255,255,255), 1)
    ...
    def find_closest_keypoint(self, keypoints, target_point):
        closest_index = None
